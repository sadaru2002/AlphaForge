@echo off
echo ================================
echo AlphaForge Final Deployment
echo ================================

REM Create properly formatted SSH key
(
...
ssh -i %TEMP%\oracle.key -o StrictHostKeyChecking=no ubuntu@161.118.218.33 "cd /opt/alphaforge && sudo docker build -t alphaforge-backend ."

echo.
echo [2/4] Stopping old container...
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "sudo docker stop alphaforge-backend 2>/dev/null || true"
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "sudo docker rm alphaforge-backend 2>/dev/null || true"
...
echo.
echo Checking logs...
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "sudo docker logs alphaforge-backend 2>&1 | tail -30"
...
echo Health: http://161.118.218.33:5000/health
echo.
echo IMPORTANT: Open port 5000 in Oracle Cloud Console
echo (See walkthrough.md for firewall instructions)
echo ================================
