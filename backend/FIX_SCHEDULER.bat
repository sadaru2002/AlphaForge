@echo off
echo ================================
echo Fixing Scheduler (Force Rebuild)
echo ================================
...
echo.
echo [1/3] Rebuilding Docker image (No Cache)...
echo This will take some time as it recompiles dependencies.
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "cd ~/alphaforge && sudo docker build --no-cache -t alphaforge-backend ."

echo.
echo [2/3] Restarting Containers...
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "sudo docker stop alphaforge-backend alphaforge-scheduler 2>/dev/null || true"
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "sudo docker rm alphaforge-backend alphaforge-scheduler 2>/dev/null || true"
