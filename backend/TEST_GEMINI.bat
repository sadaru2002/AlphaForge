@echo off
echo Testing Gemini Validation by Triggering Signal Generation...
echo.
...
echo [1] Triggering Manual Signal Generation...
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "curl -X POST http://localhost:5000/api/signals/enhanced/generate 2>&1 | head -c 500"

echo.
echo.
echo [2] Checking Backend Logs for Gemini Activity (last 30 lines):
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "sudo docker logs alphaforge-backend 2>&1 | tail -30 | grep -E 'Gemini|gemini|validation|AI'"

del %TEMP%\oracle.key
echo.
echo If you see Gemini messages above, validation is working!
echo If empty, Gemini may only run when signals pass initial filters.
