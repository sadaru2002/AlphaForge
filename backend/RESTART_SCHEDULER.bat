@echo off
echo Restarting scheduler with new API keys...
...
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "sudo docker restart alphaforge-scheduler"
timeout /t 10 /nobreak >nul
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "sudo docker logs alphaforge-scheduler 2>&1 | tail -15"

del %TEMP%\oracle.key
echo.
echo System fully configured with API keys!
