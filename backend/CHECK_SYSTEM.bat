...
if exist "..\\.env" (
    ...
    echo [LOCAL] Checking for OANDA keys...
    findstr /C:"OANDA_API_KEY" "..\\.env" >nul
    if errorlevel 1 (
        echo [ERROR] OANDA_API_KEY not found in local .env
    ) else (
        echo [OK] OANDA_API_KEY exists in local .env
    )
    findstr /C:"OANDA_ACCOUNT_ID" "..\\.env" >nul
    if errorlevel 1 (
        echo [ERROR] OANDA_ACCOUNT_ID not found in local .env
    ) else (
        echo [OK] OANDA_ACCOUNT_ID exists in local .env
    )
) else (
    echo [ERROR] .env file not found in parent directory
    echo [ACTION] Create .env file with your API keys
)
...
echo [VM] Checking containers...
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "sudo docker ps --format 'table {{.Names}}\t{{.Status}}'"

echo.
echo [VM] Checking environment in backend container...
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "sudo docker exec alphaforge-backend printenv | grep -E 'OANDA|GEMINI' | sed 's/=.*/=***HIDDEN***/'"

echo.
echo [VM] Checking scheduler logs (last 5 lines)...
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "sudo docker logs alphaforge-scheduler 2>&1 | tail -5"
...
echo Next Steps:
echo ================================
echo 1. If OANDA keys are missing locally, add them to .env
echo 2. If OANDA keys exist locally but not on VM, run UPLOAD_ENV.bat
echo 3. System is already LIVE and analyzing markets!
echo ================================
pause
