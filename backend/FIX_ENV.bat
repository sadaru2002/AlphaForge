...
echo.
echo [2/3] Starting container with .env file...
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "cd ~/alphaforge && sudo docker run -d --name alphaforge-backend --restart always -p 5000:5000 --env-file .env alphaforge-backend"

echo.
echo [3/3] Verifying connection...
timeout /t 5 /nobreak >nul
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "curl -s http://localhost:5000/api/status"

del %TEMP%\oracle.key

echo.
echo ================================
echo ✅ Redeployed with API Keys!
echo ================================
