...
echo Opening port 5000 via iptables...
ssh -i %TEMP%\oracle.key -o StrictHostKeyChecking=no ubuntu@161.118.218.33 "sudo iptables -I INPUT -p tcp --dport 5000 -j ACCEPT"

echo Verifying firewall rule...
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "sudo iptables -L INPUT -n | grep 5000"
...
echo ================================
echo Test: http://161.118.218.33:5000/health
echo ================================
