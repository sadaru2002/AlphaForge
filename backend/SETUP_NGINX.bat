...
ssh -i %TEMP%\oracle.key -o StrictHostKeyChecking=no ubuntu@161.118.218.33 "sudo apt-get update && sudo apt-get install -y nginx"

echo.
echo [2/4] Configuring Reverse Proxy...
REM Create Nginx config for forwarding port 80 to 5000
(
echo server {
echo     listen 80;
echo     server_name _;
...
scp -i %TEMP%\oracle.key nginx_config ubuntu@161.118.218.33:~/default
del nginx_config

ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "sudo mv ~/default /etc/nginx/sites-available/default && sudo nginx -t && sudo systemctl restart nginx"

echo.
echo [3/4] Opening Port 80 in VM Firewall...
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "sudo iptables -I INPUT -p tcp --dport 80 -j ACCEPT"

echo.
echo [4/4] Verifying...
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "curl -I http://localhost"
...
echo ================================
echo Access your backend here (no port needed):
echo http://161.118.218.33/health
echo ================================
