@echo off
echo ================================
echo Deploying Full System (Live Trading)
echo ================================
...
echo.
echo [1/6] Uploading updated files...
scp -i %TEMP%\oracle.key signal_scheduler.py ubuntu@161.118.218.33:~/alphaforge/
scp -i %TEMP%\oracle.key requirements_optimized.txt ubuntu@161.118.218.33:~/alphaforge/
...
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "sudo docker network create alphaforge-net 2>/dev/null || true"

echo.
echo [4/6] Restarting Backend on Network...
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "sudo docker stop alphaforge-backend 2>/dev/null || true && sudo docker rm alphaforge-backend 2>/dev/null || true"
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "cd ~/alphaforge && sudo docker run -d --name alphaforge-backend --network alphaforge-net --restart always -p 5000:5000 --env-file .env alphaforge-backend"

echo.
echo [5/6] Starting Scheduler Container...
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "sudo docker stop alphaforge-scheduler 2>/dev/null || true && sudo docker rm alphaforge-scheduler 2>/dev/null || true"
...
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "sudo docker logs alphaforge-scheduler 2>&1 | tail -20"
