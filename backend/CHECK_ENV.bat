@echo off
echo Checking Environment and Status...
...
echo.
echo [1/2] Checking OANDA_API_KEY inside container...
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "sudo docker exec alphaforge-backend printenv | grep OANDA_API_KEY"

echo.
echo [2/2] Triggering status update via API...
