@echo off
echo ================================
echo Moving to Snap-Accessible Location
echo ================================
...
echo.
echo Moving files to home directory (snap accessible)...
ssh -i %TEMP%\oracle.key -o StrictHostKeyChecking=no ubuntu@161.118.218.33 "cp -r /opt/alphaforge ~/alphaforge"

echo.
echo Building Docker image in home directory...
...
echo.
echo Checking logs...
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "sudo docker logs alphaforge-backend 2>&1 | tail -40"
...
echo.
echo Remember to open port 5000 in Oracle Cloud!
echo ================================
