...
echo.
echo [1/2] Uploading .env file...
scp -i %TEMP%\oracle.key ..\.env ubuntu@161.118.218.33:~/alphaforge/.env

echo.
echo [2/2] Restarting backend to apply changes...
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "sudo docker restart alphaforge-backend"

echo.
echo Verifying status...
timeout /t 5 /nobreak >nul
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "sudo docker logs alphaforge-backend 2>&1 | tail -20"

del %TEMP%\oracle.key

echo.
echo ================================
echo ✅ Keys Added & System Restarted!
echo ================================
echo Check health: http://161.118.218.33/health
echo ================================
