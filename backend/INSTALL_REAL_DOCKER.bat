@echo off
echo ================================
echo Installing Standard Docker
echo ================================
...
echo.
echo [1/8] Removing snap Docker...
ssh -i %TEMP%\oracle.key -o StrictHostKeyChecking=no ubuntu@161.118.218.33 "sudo snap remove docker"

echo.
echo [2/8] Installing Docker dependencies...
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "sudo apt-get update && sudo apt-get install -y ca-certificates curl gnupg lsb-release"

echo.
echo [3/8] Adding Docker repository...
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "curl -fsSL https://download.docker.com/linux/ubuntu/gpg | sudo gpg --dearmor -o /usr/share/keyrings/docker-archive-keyring.gpg"

echo.
echo [4/8] Adding Docker source...
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "echo 'deb [arch=amd64 signed-by=/usr/share/keyrings/docker-archive-keyring.gpg] https://download.docker.com/linux/ubuntu focal stable' | sudo tee /etc/apt/sources.list.d/docker.list"

echo.
echo [5/8] Installing Docker...
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "sudo apt-get update && sudo apt-get install -y docker-ce docker-ce-cli containerd.io"

echo.
echo [6/8] Building image...
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "cd ~/alphaforge && sudo docker build -t alphaforge-backend ."
...
echo.
echo [8/8] Verifying...
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "sudo docker ps && sudo docker logs alphaforge-backend 2>&1 | tail -20"
...
echo ================================
echo http://161.118.218.33:5000/health
echo ================================
