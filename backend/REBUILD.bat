@echo off
echo Uploading fixed requirements and rebuilding...
...
echo Uploading fixed requirements...
scp -i %TEMP%\oracle.key requirements_optimized.txt ubuntu@161.118.218.33:~/alphaforge/

echo Rebuilding image...
ssh -i %TEMP%\oracle.key ubuntu@161.118.218.33 "cd ~/alphaforge && sudo docker build -t alphaforge-backend . && sudo docker run -d --name alphaforge-backend --restart always -p 5000:5000 alphaforge-backend && sudo docker ps && sudo docker logs --tail 20 alphaforge-backend"
