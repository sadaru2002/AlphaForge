...
const WebSocketPrice = () => {
  ...
  const fetchRealPrice = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        console.log(`🔄 Price Update #${updateCount + 1}: ${newPrice.toFixed(5)} (${newChange >= 0 ? '+' : ''}${newChange.toFixed(1)} pips) - ${responseTime}ms`);
      } else {
  ...
  useEffect(() => {
    ...
    // ULTRA-FAST updates every 200ms (0.2 seconds) - 5 times per second!
    intervalRef.current = setInterval(fetchRealPrice, 200);
  ...
  // Force refresh every 5 seconds to prevent stale data
  useEffect(() => {
    ...
    <div className="bg-gray-800 rounded-lg p-4 border border-gray-700">
      ...
      <div className="text-xs text-gray-400 mt-2">
        ...
        <div className="text-xs text-gray-500 mt-1">
          Updates: {updateCount} | Errors: {errorCount}
        </div>
      </div>
    </div>
  );
